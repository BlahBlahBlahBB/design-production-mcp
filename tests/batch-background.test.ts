...
test("data-driven templates use one rollback-safe variant generator", async () => {
  ...
  assert.match(variants, /executeJsx\(copyDataSourceJsx/);
  assert.match(variants, /DATA_SOURCE_UNREADABLE/);
  assert.match(variants, /data_source_ms/);
  ...
  assert.match(installer, /Do not call .*set_typography.*list_text_frames.*modify_objects/);
  ...
  assert.match(installer, /DATA_SOURCE_UNREADABLE/);
  assert.match(installer, /stop and report that structured error/);
