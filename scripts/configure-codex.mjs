...
- ILLUSTRATOR OUTPUT DOMINATES COMPANION-FILE DISCOVERY: if the requested end result is an Illustrator/AI edit or generated AI file, the FIRST backend action must still be a \`design-production-illustrator\` MCP call even when the user also mentions a local folder, spreadsheet, CSV, images, or other companion files. Do not run shell commands, filesystem search, Spreadsheet skills, Python, or Computer Use before the first MCP call just to discover those files. Prefer task-level MCP tools that can resolve companion files relative to the active Illustrator document.
...
- For one-template-plus-many-data jobs such as name tags, badges, table cards, certificates, SKU labels, numbered layouts, or spreadsheet-driven variants, the FIRST backend action should normally be \`generate_template_variants\`. For the common case where the active Illustrator template and exactly one XLSX/XLS/CSV live in the same folder, omit \`data_source.file_path\`; the tool auto-discovers and copies that sibling spreadsheet through Illustrator itself. Therefore do not run \`pwd\`, \`rg\`, \`find\`, \`ls\`, \`file\`, Spreadsheet editing skills, Python/openpyxl, unzip/XML inspection, Files search, or Computer Use before this call merely to locate the template or list. For a one-binding template with exactly one editable TextFrame, also omit \`source_uuid\` and let the tool auto-bind it; do not call \`get_document_structure\`, \`get_document_info\`, \`get_text_frame_detail\`, \`list_text_frames\`, or \`get_artboards\` first. Do not loop \`duplicate_active_artboard\`, \`duplicate_objects\`, \`manage_artboards\`, or per-variant text writes. Put requested per-script fonts, paragraph alignment, conditional font sizes, and TextFrame-to-artboard centering directly into each binding so the same call finishes typography and positioning. Font matching is performed inside the batch tool with exact or unique normalized matching; do not call \`list_fonts\` proactively. Only use one filtered \`list_fonts\` call after an explicit \`FONT_NOT_FOUND\` or \`FONT_AMBIGUOUS\` result. Do not call \`set_typography\`, \`list_text_frames\`, or \`modify_objects\` afterward unless the batch result explicitly reports a formatting gap that the binding schema cannot express.
- When a spreadsheet/CSV is only a data source for template variants, prefer \`generate_template_variants.data_source\` so extraction happens inside the MCP in one pass. If the tool reports \`DATA_SOURCE_UNREADABLE\`, stop and report that structured error instead of launching a multi-step fallback chain; do not silently turn one batch request into shell probes plus spreadsheet parsing.
- Do not repeatedly call \`get_document_structure\`, \`get_images\`, \`get_text_frame_detail\`, or \`illustrator_status\` between chunks merely to confirm progress. Reuse UUIDs and geometry from the initial successful snapshot and batch-tool results; perform at most one final verification read unless a tool reports an explicit partial failure or invalidated target.
- After a successful \`generate_template_variants\` result with zero verification mismatches followed by a successful \`save_document\`, stop. Do not run post-save \`list_text_frames\`, \`get_artboards\`, filesystem \`stat\`, or other confirmation probes unless save itself reports failure or the batch result is partial/ambiguous.
