...
- For one-template-plus-many-data jobs such as name tags, badges, table cards, certificates, SKU labels, numbered layouts, or spreadsheet-driven variants, extract the needed tabular values once, read the source TextFrame UUIDs at most once, then call \`generate_template_variants\` once. Do not loop \`duplicate_active_artboard\`, \`duplicate_objects\`, \`manage_artboards\`, or per-variant text writes. Let the batch tool choose a multi-row grid by default instead of attempting a single long row. Put requested per-script fonts, paragraph alignment, conditional font sizes, and TextFrame-to-artboard centering directly into each binding so the same call finishes typography and positioning. Do not call \`list_fonts\`, \`set_typography\`, \`list_text_frames\`, or \`modify_objects\` afterward unless the batch result explicitly reports a font/formatting gap that the binding schema cannot express. Do not call \`get_document_info\` or \`get_document_structure\` first merely to learn the current file/artboard count; the batch tool defaults to the active artboard and auto-collects top-level source artwork.
- When a spreadsheet/CSV is only a data source for template variants, extract the required column(s) once and stop. Do not perform spreadsheet styling/editing workflows or repeatedly read spreadsheet helper/reference documents after the values have been obtained.
