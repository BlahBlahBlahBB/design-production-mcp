...
- For one-template-plus-many-data jobs such as name tags, badges, table cards, certificates, SKU labels, numbered layouts, or spreadsheet-driven variants, call \`generate_template_variants\` as the main operation. When the values live in XLSX/XLS/CSV, pass the file through its \`data_source\` input so the MCP parses the sheet once; the tool itself retries unreadable source paths by asking Illustrator to copy the data file to a temporary path, so do not run shell \`ls\`/\`file\`, Spreadsheet editing skills, Python/openpyxl, unzip/XML inspection, or duplicate spreadsheet parsing after the first call merely to work around path access. For a one-binding template with exactly one editable TextFrame, omit \`source_uuid\` and let the tool auto-bind it; do not call \`get_document_structure\`, \`get_document_info\`, \`get_text_frame_detail\`, \`list_text_frames\`, or \`get_artboards\` first merely to discover that target or the active artboard. Do not loop \`duplicate_active_artboard\`, \`duplicate_objects\`, \`manage_artboards\`, or per-variant text writes. Let the batch tool choose a multi-row grid by default instead of attempting a single long row. Put requested per-script fonts, paragraph alignment, conditional font sizes, and TextFrame-to-artboard centering directly into each binding so the same call finishes typography and positioning. Font matching is performed inside the batch tool with exact or unique normalized matching; do not call \`list_fonts\` proactively. Only use one filtered \`list_fonts\` call after an explicit \`FONT_NOT_FOUND\` or \`FONT_AMBIGUOUS\` result. Do not call \`set_typography\`, \`list_text_frames\`, or \`modify_objects\` afterward unless the batch result explicitly reports a formatting gap that the binding schema cannot express.
