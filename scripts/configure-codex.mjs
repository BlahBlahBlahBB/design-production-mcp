...
- For one-template-plus-many-data jobs such as name tags, badges, table cards, certificates, SKU labels, numbered layouts, or spreadsheet-driven variants, call \`generate_template_variants\` as the main operation. When the values live in XLSX/XLS/CSV, pass the file through its \`data_source\` input so the MCP parses the sheet once; do not invoke Spreadsheet editing skills, Python/openpyxl, unzip/XML inspection, or duplicate spreadsheet parsing just to obtain the values. For a one-binding template with exactly one editable TextFrame, omit \`source_uuid\` and let the tool auto-bind it; do not call \`get_document_structure\`, \`get_document_info\`, \`get_text_frame_detail\`, \`list_text_frames\`, or \`get_artboards\` first merely to discover that target or the active artboard. Do not loop \`duplicate_active_artboard\`, \`duplicate_objects\`, \`manage_artboards\`, or per-variant text writes. Let the batch tool choose a multi-row grid by default instead of attempting a single long row. Put requested per-script fonts, paragraph alignment, conditional font sizes, and TextFrame-to-artboard centering directly into each binding so the same call finishes typography and positioning. Do not call \`list_fonts\`, \`set_typography\`, \`list_text_frames\`, or \`modify_objects\` afterward unless the batch result explicitly reports a font/formatting gap that the binding schema cannot express.
- When a spreadsheet/CSV is only a data source for template variants, prefer \`generate_template_variants.data_source\` so extraction happens inside the MCP in one pass. Only use an external spreadsheet workflow if the batch tool reports that the sheet/column is ambiguous or unsupported.
