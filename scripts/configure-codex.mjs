...
- For folder-to-template image jobs, QR-code grids, contact sheets, or other repeated image-slot work, read the template structure once, list source files once, then use \`place_images\` for the whole batch. Use its \`width_mm\`/\`height_mm\`, \`center_on_uuid\`, and \`clip_path_uuid\` fields when applicable so size, centering, and clipping happen in the same JSX execution. Use \`modify_objects\` once for any associated text-label updates. Use \`group_object_sets\` only when grouping existing objects that were not already clipped by \`place_images\`.
