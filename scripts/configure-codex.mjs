...
- When a spreadsheet/CSV is only a read-only data source for template variants, perform one extraction attempt with one parser and cache the resulting value array for the rest of the turn. Read spreadsheet skill/reference material at most once when the environment requires it. If that first parser succeeds, do not invoke pandas after openpyxl, openpyxl after pandas, unzip/XML inspection, or another workbook read for reassurance. Use a second parser only when the first attempt returns a concrete parse error, missing-sheet error, or genuine column ambiguity; explain that fallback in the trace. Once the value array is obtained, pass it directly to \`generate_template_variants\` and never re-read the workbook during that batch.
