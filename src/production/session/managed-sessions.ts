...
import { createVerifiedFilesystemWorkCopy, type VerifiedFilesystemCopyResult, type WorkCopyVerification } from "./work-copy-filesystem.js";
...
export class ManagedSessionRegistry {
  ...
  async createWorkCopy(masterPath: string, workPath: string): Promise<MutationResult<{ sessionId: string; masterPath: string; workPath: string; verification: WorkCopyVerification }>> {
    ...
    try {
      ...
      const opened = await production.openDocument(workPath);
      if (!opened.ok || !opened.value) return { ok: false, contextState: "QUARANTINED", error: this.error("create-work-copy", opened.error ?? "WORK_COPY_OPEN_FAILED", opened.error?.includes("ILLUSTRATOR_TIMEOUT") ? "WORK_COPY_OPEN_TIMEOUT" : "WORK_COPY_OPEN_FAILED") };
