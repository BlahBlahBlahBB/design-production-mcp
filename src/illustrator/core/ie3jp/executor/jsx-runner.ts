...
let serializedExecution: Promise<void> = Promise.resolve();
function jsxLimit<T>(operation: () => Promise<T>): Promise<T> {
  const scheduled = serializedExecution.then(operation, operation);
  serializedExecution = scheduled.then(() => undefined, () => undefined);
  return scheduled;
}
...
export function setAppVersion(version: string | undefined, platform?: string): void {
  _sessionAppPath = version ? resolveVersionToPath(version, platform) : DEFAULT_STABLE_APP_PATH;
}
