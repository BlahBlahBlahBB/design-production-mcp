...
This record covers Phase 2 imports. It is intentionally narrower than an
upstream repository checkout: DPM retains its own transport, MCP registration,
session management, ObjectLocator, MASTER/work-copy, and quarantine behavior.
...
All imported mutation JSX is routed through
`src/illustrator/legacy/adapters/safe-donor-operation.ts`. The adapter requires
an existing `SafeMutationContext`, proves the active document is the authorized
work-copy path before donor JSX runs, quarantines an unknown transport outcome,
  and marks structural writes stale. The approved narrow public tools are
  `pathfinder_objects`, `expand_objects`, `place_image`, `relink_image`,
  `rearrange_artboards`, and `fit_artboard_to_objects`; none accepts arbitrary
  JSX, shell input, or an arbitrary Illustrator menu-command name.
